...
According to the difference of starting point for each technique and thinking way, the vessel extraction methods can be divided into these categories: pattern recognition techniques (PR), deformable model based techniques~\cite{Deformable2,Deformable3}, tracking-based techniques~\cite{Tracking2,Tracking3,Tracking5}, artificial intelligence based techniques, neural network based techniques, and miscellaneous tube-like object detection techniques. Each one contains many sub-types such as multi-scale approaches, mathematical morphology approaches. Readers could refer to~\cite{Kirbas} for an overview.
...
We first apply the enhancement of radiography based on Musicale Retinex with Color Restoration(MSRCR)~\cite{rahman1996multi}. Then, we use the gain/offset method to fix the negative values. After the pre-processing procedure, original images are enhanced in contrast and lumen, providing better basis for vessel extraction.
...
LAD and LCX branches are also as important as the catheter. Since we have a known 3D ground truth, we can easily achieve the 2D projected geometrical and structural information of LAD and LCX which we call \textit{Template} described in Fig~\ref{fig:skel_organization}(b). We take the template $t$ as ground truth and compute the similarity for each segment $s$ by $D_t(s)$ which is composed of ICP-returned parameters in the extracted image. The $D_t(s)$ is described as:

\begin{subequations}
\begin{equation}
\label{eq:dp_fp}
D_t(s) = Length_t(s) * Trans_t(s) * Rot_t(s)/Count(s),
\end{equation}

\begin{equation}
\label{eq:dp_fp_length}
Length_t(s) = |L(s)/L(t)-1|+1,
\end{equation}

\begin{equation}
\label{eq:dp_fp_trans}
Trans_t(s) = T(s)+Err(s)*THRE\_ICP,
\end{equation}
...
\end{equation}
\end{subequations}
in which $L(s)$ stands for the length of segment $s$, $T(s)$, $Err(s)$ and $R(s)$ are parameters calculated for $s$ from ICP, $THRE\_ICP$ is a constant and $Count(s)$ stands for points count of segment $s$. Eq.~\ref{eq:dp_fp_length} is used to make sure the similar length of labeling and the ground segment. Eq.~\ref{eq:dp_fp_trans} is used to evaluate transformations in image pairs. Eq.~\ref{eq:dp_fp_rot} indicates the rotation from labeling to ground truth segments.

Once we calculate the $D_t(s)$ for each segment, we obtain corresponding segments for the template shown colored in red in Fig.~\ref{fig:skel_organization}(c). Meanwhile, since we build catheter from the messy segments, we have the straight issue for building LAD branch as catheter is directly intervened into the LAD branch. We start from the intersection segment and search for the neighbours for each working node until it is a distal node. As there are multiple distal nodes during searching, there are several options LAD may be. After LAD has been determined, the same procedure advances the same for LCX branch. Suppose there are $m$ choices for LAD and $n$ choices for LCX, there are totally $m \times n$ options for the whole combinations. We transform the messy data into vessel trees according to selected LAD and LCX(See Section~\ref{subsec:vessel-tree-building}), and iterate all the $m \times n$ combinations, calculating the globally optimized energy(See Section~\ref{subsec:labeling-using-bp}) for each combination. At last, we select combination with minimal energy as the final structure. The whole algorithm is described in Algorithm~\ref{alg:icp_matching}.
...
\begin{algorithm}
  ...
  \State $sFinal \gets$ \Call{Merge}{$LADs(mMin),LCXs(nMin)$}
  \EndFunction
  \State
...
%\begin{algorithm}
%\caption{Collecting from A Given Segment}
%\label{alg:icp_collectpaths}
%  \renewcommand{\algorithmicrequire}{\textbf{Input:}}
%  \renewcommand{\algorithmicensure}{\textbf{Output:}}
%  \begin{algorithmic}
%  \Require ~~\\
%    $LNodes$, to be labeled segments. \\
%    $Coeff$, coefficients between segments.
%  \Ensure ~~\\
%    grouped segments from a given start.
%  \Function {collectPaths}{$LNodes$,$Coeff$}
%  \State $Neis \gets$ \Call{getNeighbors}{$LNodes(k)$}
%  \State $vRet \gets$ $empty$
%    \For{$i = 0 \to Count(Neis)$}
...
We define $D_{p}(f_{p})$ as the minimal normalized value the same as Eq.~\ref{eq:dp_fp}, which we call as a \textit{Distance Term}:
\begin{equation}
...
%  \Ensure ~~\\
%    built vessel tree
%  \Function {vesselTreeBP}{$mSegs$,$mRoot$}
%  \State $parent \gets$ \Call{Parent}{$mRoot$}
%  \State $children \gets$ \Call{Child}{$mRoot$}
%  \For{$child$ in $children$}
%  \State \Call{message}{$child$, $parent$}
%  \EndFor
%  \For{$child$ in $children$}
%  \State \Call{vesselTreeBP}{$mSegs$, $child$}
%  \EndFor
%  \EndFunction
%  \end{algorithmic}
%\end{algorithm}
...
\section{Conclusion}
We have developed a novel coronary artery labeling sytem from X-ray angiograms. The uniqueness of our system is its simultaneous handling on labeling as well as several applications inside X-ray angiograms. The technical cores of our system are the parallel algorithms towards
great performance. At the vessel tree building stage, we transform extracted messy segments into well organized tree structures. At labeling stage, we formulate the labeling problem as an energy optimization problem solved by belief propagation without feature tracking or registration. Besides, we proposed three applications to show the correctness as well as expandability of our method. The experimental results have shown that our method is robust for noise and even incomplete data because of the algorithms' global optimization nature. Our immediate goal for ongoing work is to continue to improve the performance and its functionalities towards clinic
